...
import { ConfigCategory, CmsToggleRow } from '../cms-ui';
...
export const SiteTab: React.FC<SiteTabProps> = ({
  ...
  const previewLinkStyle = useMemo(
    () =>
      ({
        ...
        lineHeight: 1,
        color: 'var(--foreground-brand-resting)',
        textDecoration: 'none',
        whiteSpace: 'nowrap',
        padding: '6px 4px',
        borderRadius: 4,
  ...
  return (
    <>
      <ConfigCategory title={playgroundConfig.name}>
        <p
          style={{
            margin: 0,
            font: 'var(--font-body-default-s)',
      ...
      <ConfigCategory title="Brand preset">
        <p
          style={{
            margin: '0 0 14px',
            font: 'var(--font-body-default-s)',
        ...
        >
          ...
          >
            <span style={{ color: 'var(--foreground-restrained)' }}>Current: </span>
            <span style={{ font: 'var(--font-body-strong-s)', color: 'var(--foreground-resting)' }}>
              {currentPresetDisplayLine(brandStyleSiteLabel)}
            </span>
          </div>
          <K15tButton
            type="button"
